...
class Submixes(object):
    ...
    def __init__(self, base_dir, submix_file):
        ...
        self.submix_data = yaml.safe_load(open(submix_file, 'r'))
    ...
    def do_submix(self, srcs_dir):
        print("do_submix: {}".format(srcs_dir))
        track_metadata = yaml.safe_load(open(os.path.join(srcs_dir, 'metadata.yaml'), 'r'))
        submix_dir = os.path.join(srcs_dir, _file_ready_string('submix'))
        os.makedirs(submix_dir, exist_ok=True)

        mix, mix_samplerate = sf.read(os.path.join(srcs_dir, 'mix.flac'))
        ...
        for s in os.listdir(os.path.join(srcs_dir, 'stems')):
            ...
            audio_src, stem_samplerate = sf.read(src_path)

            # Figure out which submix this source belongs to
            src_id = os.path.splitext(s)[0]
            # print(src_id)
            # print(track_metadata)
            stems_metadata = track_metadata['stems']
            if self.submix_key in stems_metadata[src_id]:
                # print(stems_metadata[src_id])
                src_submix_name = stems_metadata[src_id][self.submix_key]
                key = self._inv_sm[src_submix_name] if src_submix_name in self._inv_sm else self.RESIDUALS_KEY
            else:
                key = self.RESIDUALS_KEY
            ...
            submixes_dict[key].append(audio_src)
        ...
        for src_name, src_data in submixes_dict.items():
            ...
            if len(src_data) <= 0:
                src_data = np.zeros((1, mix.shape[0]))

            # The files should already be normalized in the mix,
            # so no need to remix/renormalize them here.
            src_path = os.path.join(submix_dir, '{}.flac'.format(src_name))
            submix = np.sum(src_data, axis=0)
            sf.write(src_path, submix, mix_samplerate)


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('-submix-definition-file', '-m', type=str, required=True,
    ...
    if args.input_dir is None and args.src_dir is None:
        raise ValueError('Must provide one of (root_dir, src_dir).')
    elif args.input_dir is not None and args.src_dir is not None:
        raise ValueError('Must provide only one of (root_dir, src_dir).')

    elif args.input_dir:
        sm = Submixes(args.input_dir, args.submix_definition_file)
